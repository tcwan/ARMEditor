...
public class MyKeywordAtOffsetHelper {
	public Pair resolveKeywordAt(XtextResource resource, int offset) {
	    ...
	    if (parseResult != null) {
	      ...
	      if (leaf != null && leaf.getGrammarElement() instanceof RuleCall) {
		        RuleCall ruleCall = (RuleCall) leaf.getGrammarElement();
		        return Tuples.create((EObject) ruleCall, (IRegion)new Region(leaf.getOffset(), leaf.getLength()));
		      }
	    }
	    return null;
	 }
